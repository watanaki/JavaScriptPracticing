...
const getHighestDuplicates = (arr) => {
  ...
  for (let num of arr) {
    ...
    sumOfAllDice += num;
    if ((count >= 3 || count >= 4) && count > highestCount)
      highestCount = count;
  }
  if (highestCount >= 4)
...
const resetGame = () => {
  diceValuesArr.fill(0);
  score = totalScore = rolls = 0;
  round = 1;
...
const checkForStraights = (arr) => {
  ...
  if (!smallStraightsArr.includes(uniqueNumbersStr) || largeStraightsArr.includes(uniqueNumbersStr))
    updateRadioOption(5, 0);
}
